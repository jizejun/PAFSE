...
parser.add_argument(
    '--use_freq_loss', action='store_true',
    help='Use frequency regularization loss'
)

parser.add_argument(
    '--freq_beta', type=float, default=1e-4,
    help='Weight of frequency regularization loss'
)
...
def learn(model=args.model,
    ...
    freq_beta = args.freq_beta if args.use_freq_loss else 0.0

    PATH = os.path.join(
        root,
        'rank{:.0f}/lr{:.4f}/batch{:.0f}/emb_reg{:.5f}/time_reg{:.5f}/freq_beta{:.1e}/'
        .format(
            rank,
            learning_rate,
            batch_size,
...
# 确定计算设备
    ...
    model = PAFSE(sizes, rank, no_time_emb=args.no_time_emb)
    model = model.cuda()
    ...
    for epoch in range(args.max_epochs):
        ...
        optimizer = TKBCOptimizer(
            ...
            freq_beta=args.freq_beta
        )



        optimizer.epoch(examples, epoch)
        ...
        if epoch < 0 or (epoch + 1) % args.valid_freq == 0:
            ...
            # Save results
            f = open(os.path.join(PATH, 'result+wuzaoting.txt'), 'a+')
